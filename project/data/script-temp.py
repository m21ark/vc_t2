...
def loadImage(num, imgLoadSizeRatio = 1, standardSize = -1):
    img = cv2.imread(os.path.join(DATA_DIR, f'{num}.jpg'))
    if img is None:
...
def inputAPI(input_file):
    try:
        with open(input_file, 'r') as f:
            ...
            # Read a CSV file instead of JSON
            df = pd.read_csv(input_file)
            return df['id'].tolist(), df['lego_count'].tolist()
...
def main():
    ...
    image_files, labels = inputAPI(input_file)
    # print("\nList of image files:", image_files)

    # Ignore the ones with label of 1
    image_files = [f for i, f in enumerate(image_files) if labels[i] != 1]
    ...
    # Load the model
    print("\nLoading model...")
    model = loadModel('vgg16-classifier.pth')
    print("Model loaded.")
    ...
    for i in range(len(preds)):
        if preds[i] != labels[i]:
            errors += abs(preds[i] - labels[i])
    
    print(f"ResNet18 num errors: {errors}")
    print(f"ResNet18 average error: {errors / len(preds)}")
